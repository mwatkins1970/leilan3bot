...
def clean_response(text: str, prompt: str, last_user: str = "") -> str:
    ...
    processed_text = re.sub(r'<[\w._-]{1,20}$', '', processed_text)
...
async def format_ensemble(
    ...
    # Convert ensemble to an async iterator if it's not already one
    ...
    async for subensemble in ensemble_iterator:
        if isinstance(subensemble, Action):
            ...
            for line in lines:
                ...
                speaker_match = re.match(r'^([\w._-]+):', line)
                if speaker_match:
                    # If we have a list in progress, add it before changing speakers
                    if current_list:
                        if current_speaker:
                            formatted_lines.append(f"<{current_speaker}>: {' '.join(current_list)}")
                        else:
                            formatted_lines.append(' '.join(current_list))
                        current_list = []
                    
                    current_speaker = speaker_match.group(1)
                    # Remove the original speaker prefix and format with angle brackets
                ...
                if list_match:
                    if not current_list:  # Starting a new list
                        current_list = [line]
                    else:  # Continuing existing list
                        current_list.append(line)
                else:
                    # Not a list item - if we have a list pending, add it
                    if current_list:
                        if current_speaker:
                            formatted_lines.append(f"<{current_speaker}>: {' '.join(current_list)}")
                        else:
                    ...
                    # Add the regular line with speaker prefix if needed
                    if current_speaker:
                        formatted_lines.append(f"<{current_speaker}>: {line}")
                    else:
            ...
            string = local_format.separator.join(formatted_lines)
        else:
...
async def generate_response(
    ...
    while retry and tries < MAX_RETRIES:
        ...
        try:
            ...
            print(f"Error generating response (attempt {tries}): {str(e)}")
